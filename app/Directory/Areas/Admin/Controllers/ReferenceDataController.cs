...
public class ReferenceDataController : Controller
{
  ...
  private readonly IReferenceDataCrudService<AssociatedDataProcurementTimeframe>
    _associatedDataProcurementTimeframeService;
  ...
  public ActionResult AddRefDataSuccessFeedbackAjax(RefDataFeedbackModel feedback)
  {
    this.SetTemporaryFeedbackMessage(
      $"The {feedback.RefDataType.ToLower()} \"{feedback.Name}\" has been added successfully.",
  ...
  {
    this.SetTemporaryFeedbackMessage(
      $"The {feedback.RefDataType.ToLower()} \"{feedback.Name}\" has been edited successfully.",
      FeedbackMessageType.Success);
    return Redirect(feedback.RedirectUrl);
  }
  ...
  {
    this.SetTemporaryFeedbackMessage(
      $"The {feedback.RefDataType.ToLower()} \"{feedback.Name}\" has been deleted successfully.",
      FeedbackMessageType.Success);
    return Redirect(feedback.RedirectUrl);
  }
  ...
  private static string ConvertFromIsoDuration(string bound)
  {
    ...
    var converted = string.IsNullOrEmpty(bound)
      ? ""
      : bound.Replace("P", "").Replace(bound.Last().ToString(), " " + dict[bound.Last().ToString()]);
        ...
        Task.Run(async () => new AnnualStatisticModel
          {
            ...
            AnnualStatisticGroupName = groups.Where(y => y.AnnualStatisticGroupId == x.AnnualStatisticGroupId)
              .FirstOrDefault()?.Name,
          })
          .Result
      )
    ...
    return View(new MaterialTypesModel
    {
      MaterialTypes = materialTypes.Select(x => Task.Run(
          async () => new ReadMaterialTypeModel
          {
            ...
            MaterialDetailCount = await _materialTypeService.GetMaterialTypeMaterialDetailCount(x.Id),
            UsedByExtractionProcedures = await _materialTypeService.IsMaterialTypeAssigned(x.Id)
          }))
        .Select(x => x.Result)
        .ToList()
    });
  ...
  public async Task<ActionResult> PagingatedDiseaseStatuses(int draw, int start, int length,
