var BiobankAdminsViewModel = (function () {
    function BiobankAdminsViewModel() {
        var _this = this;
        this.getAdmins = function (biobankId) {
            $.ajax({
                url: "/Profile/GetBiobankAdminsAjax/",
                data: { biobankId: biobankId, excludeCurrentUser: true, timeStamp: Date.now() },
        ...
        this.openInviteDialog = function () {
            $.ajax({
                url: "/Profile/InviteBiobankAdminAjax/",
        ...
        this.deleteAdmin = function (admin) {
            bootbox.confirm("Are you sure want to remove " + admin.name() + " as a Biobank Admin?", function (result) {
                if (result) {
                    $.ajax({
                        type: "POST",
                        url: "/Profile/DeleteBiobankAdminAjax/",
                        data: { OrganisationUserId: admin.userId, OrganisationId: _this.biobankId },
                        success: function (data) {
                            if (data.success) {
        ...
        this.elements = {
            id: "#BiobankModel_BiobankId",
            modal: "#modalInvite" + base,
            form: "#modalInvite" + base + "Form"
        };
        this.admins = ko.observableArray([]);
        this.dialogErrors = ko.observableArray([]);
        this.biobankId = $(this.elements.id).val();
        this.getAdmins(this.biobankId);
    }
    return BiobankAdminsViewModel;
})();
