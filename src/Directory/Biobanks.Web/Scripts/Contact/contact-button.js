var ContactButtonManager = (function () {
    function ContactButtonManager(localStorageContactIdsKey, contactButtonBase) {
        ...
        this.contactIds = JSON.parse(localStorage.getItem(this.localStorageContactIdsKey));
    ...
    ContactButtonManager.prototype.StyleContactButton = function (button) {
        if (this.contactIds.indexOf(button.data("biobank-id")) !== -1) {
            ...
            button.html('<span class="fa fa-envelope-o labelled-icon"></span>Remove from Contact List');
        }
        else {
            // TODO: abstract this into separate function 
            ...
            button.html('<span class="fa fa-envelope-o labelled-icon"></span>Add to Contact List');
        }
    };
    ...
    ContactButtonManager.prototype.UpdateContactCounter = function () {
        ...
        $("#ContactCounter").html(this.contactButtonBase.replace("{count}", String(count)));
    ...
    ContactButtonManager.prototype.ContactCounter_Click = function () {
        ...
        if (this.contactIds.length > 0) {
            ...
            var dialog_1 = bootbox.dialog({
                message: "In order to contact resources, please use the search box to identify " +
    ...
    ContactButtonManager.prototype.ContactButton_Click = function (e) {
        ...
        if (this.contactIds.indexOf(biobankId) === -1) {
            ...
            button.addClass("btn-danger");
            button.html('<span class="fa fa-envelope-o labelled-icon"></span>Remove from Contact List');
            window.feedbackMessage("Resource added to the contact list. Access the contact list from the button at the top right.", "success");
        }
        else {
            // Otherwise, remove it.
    ...
    ContactButtonManager.prototype.SaveToLocalStorage = function () {
        localStorage.setItem(this.localStorageContactIdsKey, JSON.stringify(this.contactIds));
    };
    return ContactButtonManager;
}());
$(function () {
    var cbm = new ContactButtonManager("contactIds", "<span class=\"fa fa-envelope-o labelled-icon\"></span>Contact List ({count})");
    cbm.Init();
});
