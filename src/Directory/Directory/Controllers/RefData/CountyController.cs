...
namespace Directory.Controllers.RefData
{
    ...
    public class CountyController : Controller
    {
        ...
        [SwaggerResponse(200, "All Counties", typeof(List<CountyDto>))]
        [HttpGet]
        public async Task<IActionResult> Index()
           => Ok(await _readService.ListCounties());

        [SwaggerOperation("Get a single County by ID")]
        [SwaggerResponse(200, "The County with the requested ID.", typeof(CountyDto))]
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CountyDto county)
        {
        ...
        [SwaggerResponse(404, "No County was found with the provided ID.")]
        [HttpPut("{id}")]
        public async Task<IActionResult> Put(int id, [FromBody] CountyDto county)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
