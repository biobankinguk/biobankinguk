...
namespace Biobanks.Directory.Services
{
    public class CountryService : ReferenceDataService<Country>
    {
        ...
        public override async Task<int> GetUsageCount(int id)
            => await _db.Countries.CountAsync(x => x.Id == id) + await _db.Counties.CountAsync(x => x.CountryId == id);

        public override async Task<bool> IsInUse(int id)
            => await _db.Countries.AnyAsync(x => x.Id == id) || await _db.Counties.AnyAsync(x => x.CountryId == id);
    }
}