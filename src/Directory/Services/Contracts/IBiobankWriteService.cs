...
namespace Directory.Services.Contracts
{
    public interface IBiobankWriteService
    {
        ...
        Task<string> StoreImageAsync(Stream image, string imageName, string contentType, string reference, int maxX = 1200, int maxY = 1200);

        Task DeleteBlobAsync(string resourceName);

        // Service Methods For Logos
        Task<string> StoreLogoAsync(Stream logoFileStream, string logoFileName, string logoContentType, string reference);
        Task DeleteLogoAsync(int organisationId);
        #endregion
    }
}
