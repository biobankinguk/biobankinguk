...
{
    public class OntologyService : IDataLoadService
    {
        ...
        private async Task SubmitData(IList<string> data)
        {
            ...
            for (var i = 0; i < data.Count; i++)
            {
                ...
                var result = await Program.SendJsonAsync("ontology", JsonConvert.SerializeObject(ontology));
        ...
        private IEnumerable<string> PrepData()
        {
            ...
            var data = new ConfigurationBuilder()
                .AddJsonFile("StaticData/Ontology.json", optional: false)
                .Build();

            var values = data.GetSection("Ontology")
                .GetChildren()
                .AsEnumerable()
                .Select(x => x.Value)
                .ToList();
