...
{
    public class SnomedTagService : IDataLoadService
    {
        ...
        private async Task SubmitData(IList<string> data)
        {
            ...
            {
                var snomedTag = data[i];

                var result = await Program.SendJsonAsync("SnomedTag", JsonConvert.SerializeObject(snomedTag));
            ...
            timer.Stop();
            Console.WriteLine($"SnomedTag Posts took: {timer.Elapsed}");
        ...
        private IEnumerable<string> PrepData()
        {
            ...
            var data = new ConfigurationBuilder()
                .AddJsonFile("StaticData/SnomedTag.json", optional: false)
                .Build();

            var values = data.GetSection("SnomedTag")
                .GetChildren()
                .AsEnumerable()
                .Select(x => x.Value)
                .ToList();
            ...
            timer.Stop();
            Console.WriteLine($"SnomedTag data prepared: {_totalRecords} record(s).");
